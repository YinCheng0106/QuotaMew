...
`ProviderWindowsPresentation` 是目前 snapshot 的純值 projection。Codex 精確 ID `codex.base_model_inference.primary/secondary`（runtime 觀察）與 `codex.gpt-reserve.primary/secondary`（相容別名）顯示為 **Luna Reserve**。一般 windows 先顯示；Reserve 平常只提供沒有 progress bar／倒數的次要精簡列。僅當 available、capture age 在 0..<15 分鐘、一般 codex bucket 的 5 小時或每週 window 原始 usedPercentage 恰為 100 且 reset 尚未到期，才展開 Reserve。這不宣稱 Reserve 已啟用、計費狀態或使用資格；缺少／stale／invalid 資料不觸發。選單列不會用 Reserve 自動取代一般 window；只有使用者在 Single 明確選取，或 Overview 沿用同一個 prominence projection 時才呈現 Reserve。沒有新增 persistence/history、refresh 或 timer。
...
### v0.2 Menu Bar Display Polish（2026-09-08，source complete，automated validation complete，manual acceptance complete）

`SettingsStore` 新增兩個 bounded typed preferences：`presentation.menu-bar.display-style`（`single`／`overview`）與 `presentation.menu-bar.quota-selection`（`fiveHour`／`weekly`／`lunaReserve`）。兩者缺少或含未知 future raw value 時，runtime 安全採用同一組預設 **Single + Weekly**，且不覆寫未知值。這讓 Beta 2 沒有新 keys 的 domain 從原本的 weekly 產品語意變為明示的 `W n%`，而不是靜默改成 5-hour；fresh domain 也採相同預設，避免兩套不可見規則。既有 Remaining／Used 與 pinned-provider keys 完全獨立。

`MenuBarPresentation` 只從 `AppModel.providerStates` 與 normalized `UsageWindow` 建立純值 projection。Single 依 `UsageWindowPresentation.menuBarQuotaSelection` 精確選出 `5H`、`W` 或 `R`；缺少該 metric 時保留選擇並顯示 `5H —`／`W —`／`R —`，不按 array position、primary/secondary role 或其他可用 window 替代。Overview 固定產生 5H、W 的順序；只有 `ProviderWindowsPresentation.showsReserveProminently` 已為 true 且有有效 Reserve usage 時才在最後加入 R。同一 semantic metric 若出現多個候選，以穩定 window ID 決定一個，不重複顯示。Claude 目前只有精確 18,000 秒與 604,800 秒 windows 可安全共用 5-hour／Weekly-like selection；其他 duration 一律 unavailable，不虛構 mapping。

每個 metric 的 Remaining／Used 值仍由 `UsagePresentation` 導出。Settings action 同步保存後只更新 `SettingsModel` 的 observable projection，既有 `StatusItemController` observation 重新套用同一個 standard button；不 refresh、不進 `UsageService`／Codex RPC、不評估通知或 reset、不建立 scheduler。Visible title 使用語言中立的 `5H`／`W`／`R`，VoiceOver value 則直接組合 `UsageWindowPresentation` 的完整在地化名稱與 `UsagePresentation` 的完整百分比敘述，不解析 compact title。

Status item 保留既有 template icon。以標準 `NSStatusBarButton`、small monospaced-digit title 與 `intrinsicContentSize` 的受控量測顯示 icon 固定增加 17.5 pt；代表性 with-icon 寬度為 Single 59.5–81.5 pt、regular Overview 110.5–138.5 pt、含 prominent Reserve 156.5–191.5 pt。保留 icon 可維持 Single／Overview 一致性、識別與點擊目標；沒有 custom view 或幾何 polling。Overview 較寬是明確使用者選擇，macOS 空間不足時沿用既有 system visibility／recovery 語意，不自動切回 Single。
...
QuotaMew 使用原生 `Settings` scene 與 `@MainActor SettingsStore`。Settings 以原生 `TabView` 固定為 General／Providers／Notifications 三頁：General 包含 app behavior、Remaining／Used、Menu Bar presentation 與 Diagnostics；Providers 保留 enablement 與 Claude Experimental / Unverified 說明；Notifications 保留所有既有 reminder 與 permission 行為。`SettingsStore` 是 provider enablement、presentation mode、menu-bar display style、menu-bar quota selection、optional pinned provider、通知總開關，以及短視窗 1 小時／30 分鐘與長視窗 24／6／1 小時門檻的單一來源，並以 typed UserDefaults keys 保存；SwiftUI 只透過 `SettingsModel` 修改 store，再由既有 consumers 讀取同一份狀態。Provider toggle 先同步通知 `NotificationService` 使舊 lifecycle 失效，再保存設定、更新 `AppModel` 投影，並於 enable 時要求共用 refresh。Menu-bar presentation setters 則只保存與更新 observable presentation，不通知 services。`SettingsModel.setProvider` 回傳 `Void`；View 只表達使用者意圖，不接收或 await cleanup task。Cleanup 可以非同步完成，因正確性由 service 的 provider generation 與 destructive-point validation 保證，不依賴 SwiftUI call site 記住實作細節。短、長視窗的 1 小時選項彼此獨立，舊版共用 1 小時偏好會作為短視窗偏好的 migration 預設。`NotificationService` 關閉時不再建立提醒，並移除 QuotaMew 自己的 pending reset requests；關閉單一門檻只移除相符 duration class 的門檻。
...
Onboarding 直接讀共用 `SettingsModel`：Remaining／Used 與 pinned provider 立即走既有 presentation observation；Launch at Login 走原 `LaunchAtLoginController` 並回讀系統狀態；通知 opt-in 走原 `NotificationService`。進階的 Single／Overview 與 quota selection 留在 Settings，不把 Onboarding 擴成完整設定頁；完成、略過與重看都保留這兩項既有值。Provider status 由既有 `CompatibilityDiagnosticsSnapshot` 產生；它可讀 provider 的 bounded runtime diagnostic snapshot，但不呼叫 `fetchUsage()`、不建立另一個 probe 或 bridge，也不產生 onboarding network traffic。Claude 永遠另顯示 Experimental / Unverified；Luna Reserve 不成為 provider 選項。
...
**CURRENT**：production composition root 只建立一個 `StatusItemController`。Controller 建立並持有唯一 `NSStatusItem`，Debug／Release 分別使用 `dev.quotapulse.development.app.primary-status-item` 與 `dev.quotapulse.app.primary-status-item` 作為 stable `autosaveName`，以一個 `.transient` `NSPopover` 和 `NSHostingController` 呈現既有 `DashboardView`；Dashboard、Settings、providers、refresh、notifications 與 shared `AppModel`／`SettingsModel` 都未複製。Status button 以既有 SF Symbol、template image、monospaced compact semantic metrics（例如 `W 71%`、`5H 86% · W 71%`）或 metric-specific unavailable placeholder 呈現，並明確設定 VoiceOver label/value。標準 `NSStatusBarButton` 使用 `.imageLeading`、`imageHugsTitle = true` 與 `.scaleProportionallyDown`；item 先以 `NSStatusItem.variableLength` 建立，再於每次內容更新時以 button 的 `intrinsicContentSize.width` 動態設定公開的 `length`，下限為目前 status bar thickness。此作法沒有固定 percentage 寬度、人工空白、kerning hack 或 custom view。
...
Controller 是選單列 shell 的唯一 observation owner：一條 Swift Observation path 讀取 persisted intent 與包含 pin、display style、quota selection、Remaining／Used 的 `MenuBarPresentation`，一條 KVO observation 讀取 system logical visibility。它以單一 bounded re-registration task 合併同一輪 presentation changes，並記住最後一次已套用的 intent，因此更新不會累積 observers／controllers，也不會把使用者或 macOS 已移除的 item 反覆插回；recovery action 則可明確 force-show，即使 persisted intent 原本已是 `true`。Open popover 只呼叫既有 `AppModel.menuDidOpen()`；沒有新增 timer、polling、network、scheduler 或 provider I/O。Teardown 會取消 observation、清除 target/action、關閉 popover並移除同一個 status item，且可重複安全呼叫。
...
**Milestone A — COMPLETE / frozen. Milestone B — COMPLETE. Milestone C — COMPLETE. Product Polish — COMPLETE. Menu Bar Display Polish — SOURCE COMPLETE / AUTOMATED VALIDATION COMPLETE / MANUAL ACCEPTANCE COMPLETE.** 使用者已回報新的 Single／Overview、三種 quota selection、最寬 Overview、英／繁中 VoiceOver、crowded／notch、互動與 regression checks 全數通過；這是 user-observed evidence，不由 deterministic tests 或 build 推導。Beta 3 release preparation 尚未開始，Reset Intelligence 仍屬 v0.3。
...
`SettingsStore` 的純本機 v0.2 persisted presentation contract 是 `remaining`／`used`（預設 `remaining`）、`single`／`overview`（預設 `single`）、`fiveHour`／`weekly`／`lunaReserve`（預設 `weekly`），以及 optional pinned provider raw value。`SettingsModel` 在啟動時建立這些偏好的唯一 runtime-observable projection；Settings action 會同步保存到 `SettingsStore` 並更新同一個 model。`StatusItemController` 直接觀察該 shared model，更新 standard status button 的 icon、compact metrics 與 accessibility metadata；SwiftUI popover content 也使用同一份 models，不複製 state。`UsagePresentation` 是唯一的 percentage projection：它從同一個 normalized `UsageWindow` 導出 rounded remaining 或 used percentage，絕不寫回 domain、reset detector 或 notification policy。Dashboard 的主要 percentage、VoiceOver value 與選單列共享它；進度與通知仍保留既有 used／remaining semantics。`MenuBarPresentation` 明確區分 persisted pinned provider、selected provider、currently rendered provider、metric selection 與 renderability：known explicit pin 只呈現該 provider 的可用 snapshot；loading/stale cached snapshot 可保留數字，disabled、unavailable、unknown future pin 或缺少 selected metric 時顯示明確 placeholder，絕不暗中切到另一個 provider／metric。無 pin 時只依既有 provider order 選擇第一個能呈現目前配置的 provider，且不寫回 preference。General／Providers／Notifications 已是唯一 Settings IA。
